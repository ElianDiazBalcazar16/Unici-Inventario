...
<dialog id="dialog" class="top-10 py-5 z-50 min-w-[400px] px-5 rounded-lg bg-slate-300 border-2 border-sky-700" style="display: none;">
    ...
    <form action="{{ route('createCampus') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                filteredInventory.forEach(item => {
                    ...
                    row.innerHTML = `
                        ...
                        <td class="flex h-auto px-4 py-2 text-sm text-gray-600 border-t border-gray-300">
                            <a href="Campus/actualizarDato/${item.id}" class="btn btn-primary " tabindex="-1" role="button" aria-disabled="true">Editar</a>
                            <form class="grid h-full place-content-center" action="/Campus/eliminarDato/${item.id}" method="POST">
                                @csrf
                                @method('PUT')
