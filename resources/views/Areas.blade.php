...
<dialog id="dialog" class="top-10 py-5 z-50 min-w-[400px] px-5 rounded-lg bg-slate-300 border-2 border-sky-700" style="display: none;">
    ...
    <form action="{{ route('createAreas') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="flex gap-8">
            <div>
                ...
                <select name="Campus" class="p-2 mt-2 border border-gray-300">
                ...
                <input type="text" name="Nombre" id="Nombre" class="px-1.5 py-1 rounded-lg border-2 border-sky-500">
                <h2 class="pl-2 mt-1 mb-1 font-semibold text-sky-800">
    ...
    document.addEventListener('DOMContentLoaded', () => {
            ...
            let inventory = [
                @foreach ($Datos as $Dato)
                {
                    id: {{ $Dato->id }},
                    idCa: {{ $Dato->Campus }},
                    NombreCampus: '{{ $Dato->NombreCampus }}',
                    Nombre: '{{ $Dato->Nombre }}',
            ...
            const renderTable = () => {
                ...
                filteredInventory.forEach(item => {
                    ...
                    row.innerHTML = `
                        <td class="px-4 py-2 text-sm text-gray-600 border border-gray-300">${item.NombreCampus}</td>
                        <td class="px-4 py-2 text-sm text-gray-600 border border-gray-300">${item.Nombre}</td>
                        <td class="px-4 py-2 text-sm text-gray-600 border border-gray-300">${item.Responsable}</td> 
                        
                        <td class="flex h-auto px-4 py-2 text-sm text-gray-600 border-t border-gray-300">
                            <a href="Areas/actualizarDato/${item.id}" class="btn btn-primary " tabindex="-1" role="button" aria-disabled="true">Editar</a>
                            <form class="grid h-full place-content-center" action="Areas/eliminarDato/${item.id}" method="POST">
                                @csrf
                                @method('PUT')
